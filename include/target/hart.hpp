...
#endif
            cur_level{PrivilegeLevel::MACHINE_MODE} {}
...
///     }
    ...
    RetT illegal_instruction(const Instruction *inst) {
        return internal_interrupt(trap::ILLEGAL_INSTRUCTION,
                                  *reinterpret_cast<const UXLenT *>(inst));
    }
    ...
    RetT visit_beq_inst(const BEQInst *inst) {
        return operate_branch<typename operators::EQ<xlen>>(inst);
    }

    RetT visit_bne_inst(const BNEInst *inst) {
        return operate_branch<typename operators::NE<xlen>>(inst);
    }

    RetT visit_blt_inst(const BLTInst *inst) {
        return operate_branch<typename operators::LT<xlen>>(inst);
    }

    RetT visit_bge_inst(const BGEInst *inst) {
        return operate_branch<typename operators::GE<xlen>>(inst);
    }

    RetT visit_bltu_inst(const BLTUInst *inst) {
        return operate_branch<typename operators::LTU<xlen>>(inst);
    }

    RetT visit_bgeu_inst(const BGEUInst *inst) {
        return operate_branch<typename operators::GEU<xlen>>(inst);
    }
    ...
    RetT visit_addi_inst(const ADDIInst *inst) {
        return operate_imm<typename operators::ADD<xlen>>(inst);
    }

    RetT visit_slti_inst(const SLTIInst *inst) {
        return operate_imm<typename operators::SLT<xlen>>(inst);
    }

    RetT visit_sltiu_inst(const SLTIUInst *inst) {
        return operate_imm<typename operators::SLTU<xlen>>(inst);
    }

    RetT visit_xori_inst(const XORIInst *inst) {
        return operate_imm<typename operators::XOR<xlen>>(inst);
    }

    RetT visit_ori_inst(const ORIInst *inst) {
        return operate_imm<typename operators::OR<xlen>>(inst);
    }

    RetT visit_andi_inst(const ANDIInst *inst) {
        return operate_imm<typename operators::AND<xlen>>(inst);
    }

    RetT visit_slli_inst(const SLLIInst *inst) {
        return operate_imm_shift<typename operators::SLL<xlen>>(inst);
    }

    RetT visit_srli_inst(const SRLIInst *inst) {
        return operate_imm_shift<typename operators::SRL<xlen>>(inst);
    }

    RetT visit_srai_inst(const SRAIInst *inst) {
        return operate_imm_shift<typename operators::SRA<xlen>>(inst);
    }

    RetT visit_add_inst(const ADDInst *inst) {
        return operate_reg<typename operators::ADD<xlen>>(inst);
    }

    RetT visit_sub_inst(const SUBInst *inst) {
        return operate_reg<typename operators::SUB<xlen>>(inst);
    }

    RetT visit_sll_inst(const SLLInst *inst) {
        return operate_reg<typename operators::SLL<xlen>>(inst);
    }

    RetT visit_slt_inst(const SLTInst *inst) {
        return operate_reg<typename operators::SLT<xlen>>(inst);
    }

    RetT visit_sltu_inst(const SLTUInst *inst) {
        return operate_reg<typename operators::SLTU<xlen>>(inst);
    }

    RetT visit_xor_inst(const XORInst *inst) {
        return operate_reg<typename operators::XOR<xlen>>(inst);
    }

    RetT visit_srl_inst(const SRLInst *inst) {
        return operate_reg<typename operators::SRL<xlen>>(inst);
    }

    RetT visit_sra_inst(const SRAInst *inst) {
        return operate_reg<typename operators::SRA<xlen>>(inst);
    }

    RetT visit_or_inst(const ORInst *inst) {
        return operate_reg<typename operators::OR<xlen>>(inst);
    }

    RetT visit_and_inst(const ANDInst *inst) {
        return operate_reg<typename operators::AND<xlen>>(inst);
    }

    RetT visit_ecall_inst(riscv_isa_unused const ECALLInst *inst) {
        switch (cur_level) {
#if defined(__RV_USER_MODE__)
            case PrivilegeLevel::USER_MODE:
                return internal_interrupt(trap::U_MODE_ENVIRONMENT_CALL, 0);
#endif
#if defined(__RV_SUPERVISOR_MODE__)
            case PrivilegeLevel::SUPERVISOR_MODE:
                return internal_interrupt(trap::S_MODE_ENVIRONMENT_CALL, 0);
#endif
            case PrivilegeLevel::MACHINE_MODE:
    ...
    RetT visit_mul_inst(const MULInst *inst) {
        return operate_reg<typename operators::MUL<xlen>>(inst);
    }

    RetT visit_mulh_inst(const MULHInst *inst) {
        return operate_reg<typename operators::MULH<xlen>>(inst);
    }

    RetT visit_mulhsu_inst(const MULHSUInst *inst) {
        return operate_reg<typename operators::MULHSU<xlen>>(inst);
    }

    RetT visit_mulhu_inst(const MULHUInst *inst) {
        return operate_reg<typename operators::MULHU<xlen>>(inst);
    }

    RetT visit_div_inst(const DIVInst *inst) {
        return operate_reg<typename operators::DIV<xlen>>(inst);
    }

    RetT visit_divu_inst(const DIVUInst *inst) {
        return operate_reg<typename operators::DIVU<xlen>>(inst);
    }

    RetT visit_rem_inst(const REMInst *inst) {
        return operate_reg<typename operators::REM<xlen>>(inst);
    }

    RetT visit_remu_inst(const REMUInst *inst) {
        return operate_reg<typename operators::REMU<xlen>>(inst);
    }
    ...
    // todo: only comparing the value, memory ordering.
